...
from agent.repair import SelfRepairEngine, SelfRepairLoop
...
"""
    ...
    def repair_repository(
        ...
        task_id: str = "repo_repair"
    ) -> dict[str, Any]:
        """
        Triggers the standalone, workspace-isolated self-repair engine over a target repository.
        """
        target_dir = os.path.abspath(repo_path) if repo_path else self.workspace_path
        self.log_event("SELF_REPAIR_STARTED", {"target_dir": target_dir, "offline_only": offline_only})
        ...
        self.log_event("SELF_REPAIR_COMPLETED", res)
        return res
    ...
    def execute_task(self, task_description: str) -> dict[str, Any]:
        ...
        except (ValueError, KeyError, SyntaxError) as cog_err:
        ...
        for iteration in range(1, max_iterations + 1):
            ...
            repair_res = self.repair_engine.repair_repository(
                repo_dir=self.workspace_path,
                max_retries=1,
                offline_only=False,
                task_id=f"engine_task_iter_{iteration}"
            )
            if repair_res.get("repaired"):
                success = True
                used_strategy = repair_res.get("strategy", used_strategy)
                self.log_event("APPLIED_REPAIR", repair_res)
                break
        ...
        if success:
            ...
            except (RuntimeError, ValueError, OSError) as git_err:
